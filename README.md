...
## 🚀 当前版本
**v4.39.3** - 多游戏 Pack 命令/端口/虚标修复（P0 + P1） (2026-07-31)
- ARK 停服丢档修复：`stop_command` 从 `quit` 改为 `DoExit`（SaveWorld + 退出）
- Palworld Steam App ID 修正（2374020 → 2394010）+ RCON 端口对齐（25575 → 25585）
- Minecraft vanilla mods `jar-rename` 虚标移除（vanilla 不支持 mods）
- Rust `give` 改为 `inventory.giveto`；Terraria/Valheim shop/cdk/welcome 禁用（原版无 give 命令）
- ARK `GiveItem` 改为 `GFI`（物品短名替代完整蓝图路径）

**上一版本 v4.39.2** - 生产运行时迁移 node dist/（build pipeline 重构） (2026-07-31)
...
- 折叠侧边栏 CSS Tooltip + 分组折叠状态 localStorage 持久化
- 侧边栏管理分组从 10 组精简为 5 组，快捷入口响应式网格优化
- WorkbenchUI 组件库全面对齐 iOS 设计规范（#007AFF 主色、pill 按钮、轻量阴影）

**v4.39.0** - 项目整改批次1：致命项修复 + 僵尸清理 (2026-07-30)
- `/guild/servers` 重做为全平台服务器市场：公开实例直接绑定 + 私有实例申请-审批流程
- 新增 binding_requests 表 + 6 个绑定申请端点 + 1 个可绑定实例列表端点
- 服主可上架/下架实例（is_public）、开启/关闭绑定申请通道（binding_requests_enabled）
- 审批通过自动创建 binding（vip_level=1，与直接绑定一致）
- 新增「我的绑定申请」页 `/guild/my-binding-requests` + 实例详情「绑定申请」Tab
- 修复 GuildBind 可绑定列表数据源（原只能看到自己创建的实例）

**v4.37.2** - 修复多角色用户切换 active_role=user 后访问自有实例 403 (2026-07-30)
- 修复 Register.tsx 注册后跳转 /dashboard（已废弃→/admin→403）导致新用户被弹到 /forbidden 的 bug
- App.tsx /dashboard 重定向目标改为 /（按角色分流），防御性修复所有残留 /dashboard 调用方
- Forbidden 页重做为 Apple 浅色风格卡片（盾牌图标 + 眉标 + 圆角阴影卡片），保留全部角色切换逻辑

**v4.37.0** - CDK 机制改造：服主发放 + 可重复使用 CDK (2026-07-30)
- 修复 Business.tsx cdk 子 Tab 组件挂载错误（原为玩家兑换，改为服主发放管理）
- 新功能：可重复使用 CDK（一次性/限N次/无限次），支持多玩家兑换同一 CDK
- 新增 cdk_redemptions 兑换记录子表，前端可展开查看兑换历史

**v4.36.1** - 修复部署后 lazy chunk 加载失败导致页面白屏 (2026-07-30)
- 新增 lazyWithRetry 工具函数：chunk 加载失败时自动刷新页面（sessionStorage 防无限循环）
- 替换 6 个文件中所有 `lazy(() => import(...))` 调用，部署后不再需要用户手动刷新
