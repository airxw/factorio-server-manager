...
- **下游接续入口**：
  ...
  - v4.27.0：B5 `<VirtualList>` 虚拟滚动组件（基于 react-window，用于 1000+ 数据场景）
...
- **已知遗留项（非本次修复范围）**：
  - SetupWizard.test.tsx 29 用例失败（预存问题：测试文件 untracked，与 v4.20.0+ 重构后的向导不同步，与 B1-B9 修复无关）
