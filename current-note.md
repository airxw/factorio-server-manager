...
## (0) 版本线
- **v4.39.3 多游戏 Pack 命令/端口/虚标修复（本地已闭合，待部署，2026-07-31）**：项目锐评识别出多个游戏 Pack 存在命令错误、端口不一致、虚标功能等问题，本次按 P0/P1 优先级修复 6 个游戏 Pack（不动 factorio）。**①ARK 停服丢档修复（P0）**：[packs/ark-vanilla/pack.yaml](packs/ark-vanilla/pack.yaml) `stop_command` 从 `quit`（不存档）改为 `DoExit`（SaveWorld + 退出），避免每次停服丢失一个 autosave 周期进度。**②ARK 物品发放命令修正（P1）**：`GiveItem` 需完整蓝图路径过长，改用 `GFI <ItemName> <Quantity> <Quality> <Blueprint>`，物品短名与 static_list.name 一致；commands.give_item + business.shop/cdk/welcome 全部同步。**③Palworld Steam App ID 修正（P0）**：原 `steamcmd://2374020` 不存在（官方实锤 2394010），三处同步——[packs/palworld-vanilla/pack.yaml](packs/palworld-vanilla/pack.yaml) versions.source / [daemon/src/steamcmd/apps.ts](daemon/src/steamcmd/apps.ts) STEAM_APP_IDS.palworld / [panel/backend/src/core/packs/staticVersions.ts](panel/backend/src/core/packs/staticVersions.ts) 注释，verified: https://docs.palworldgame.com/getting-started/deploy-dedicated-server/。**④Palworld RCON 端口对齐（P0）**：`required_ports` rcon 端口 25575 与 `protocol.default_port` 25585 不一致（v4.12.0 漏改），统一为 25585，避免与 Minecraft 25575 冲突。**⑤Minecraft vanilla mods 虚标移除（P1）**：vanilla 服务端不加载 mods/ 目录（需 Forge/Fabric），原 `mods.mechanism: jar-rename` 是虚标，[packs/minecraft-vanilla/pack.yaml](packs/minecraft-vanilla/pack.yaml) 彻底删除 `mods:` 块（schema optional），ui.tabs 已在前面移除 mods tab。**⑥Rust give 命令修正（P1）**：[packs/rust-vanilla/pack.yaml](packs/rust-vanilla/pack.yaml) `give` 命令在 vanilla Rust 不存在（uMod 插件命令），改为 `inventory.giveto {{player}} "{{item}}" {{count}}`，commands + business.shop/cdk/welcome 全部同步。**⑦Terraria vanilla shop/cdk/welcome 禁用（P1）**：原版无 give 命令（仅 TShock 支持 /give），[packs/terraria-vanilla/pack.yaml](packs/terraria-vanilla/pack.yaml) `business.shop.enabled`/`cdk.enabled`/`welcome.enabled` 全部 false，give_item 留空，ui.tabs 移除 shop-admin。**⑧Valheim vanilla shop/cdk/welcome 禁用（P1）**：原版 stdin 仅支持 say/kick/ban/unban/info/save，spawn 是客户端 devcommands，[packs/valheim-vanilla/pack.yaml](packs/valheim-vanilla/pack.yaml) 同 Terraria 禁用 shop/cdk/welcome + 移除 shop-admin tab。**不调整 factorio**：经核查 factorio-vanilla 配置正确（stop_command=quit 是 Factorio 控制台原生保存退出命令；RCON 27015 一致；mods mechanism=list-file 与 mod-list.json 一致）。**验证**：三端 tsc --noEmit 0 错误；backend vitest 709/709 PASS；daemon 20/20 PASS；frontend 328/328 PASS；9 个 pack.yaml 全部通过 `GamePackSchema.safeParse` zod 校验（loadPacksFromDir 加载成功）；Pack 关键字段核对：ark stop=DoExit / palworld rcon_port=25585（与 required_ports 一致）/ minecraft mods_mech=(none) / rust tabs 含 shop-admin（inventory.giveto）/ terraria+valheim tabs 不含 shop-admin；check:version 12 源全绿 4.39.3；BUILD_ID 升至 20260731-003；前端 build 成功 + dist 无 localhost:3000/127.0.0.1:3000 违规。**遗留（不阻断）**：Terraria TShock variant 保留 shop-admin（TShock 支持 /give）；ARK GFI 物品短名与游戏内实际名称映射需用户实测微调。未部署。
- **v4.39.2 build pipeline 重构落地 + DEF-008 修复转化（已部署，2026-07-31）**：落地 v4.39.0 登记的 build pipeline 重构 TODO——生产运行时从 `npx tsx src/index.ts` 迁移到 `node dist/index.js`（esbuild 单文件 bundle）。**①bundle 管线**：新增 [panel/backend/scripts/build-dist.mjs](panel/backend/scripts/build-dist.mjs) + [daemon/scripts/build-dist.mjs](daemon/scripts/build-dist.mjs)（`--packages=external` 保 sqlite3/knex/pino 外部依赖，`alias @public` 编译期解析）；两端 `build:bundle`/`start:dist` 脚本 + esbuild devDep。**②migrations 双模式**：build-dist.mjs 多入口编译 29 个迁移至 `dist/db/migrations/*.js`（共享 chunk 隔离 `dist/db/chunks/`）；[connection.ts](panel/backend/src/db/connection.ts) `resolveMigrationsConfig()` 按目录存在性自动切换 tsx/dev（src .ts）↔ node dist（dist .js）。**③路径修复**：daemon [ExecutionEngine.ts](daemon/src/modules/execution_engine/ExecutionEngine.ts) schema 路径从 `__dirname` 相对改 `process.cwd()` 相对（bundle 后深度变化失效）。**④deploy/systemd 切换**：deploy.sh build() 新增 `npm run build:bundle`（tsc 保留作类型门禁）；两个 systemd 单元 ExecStart → `/usr/bin/node dist/index.js`，v4.39.0 TODO 闭合。**⑤DEF-008 修复+转化**：migration 20260730000006 文件名时序先于建表迁移 20260830000002，fresh install UPDATE 不存在表崩溃（SQLITE_ERROR），up/down 加 hasTable 幂等防护；转化产物=回归测试 [fresh-install-migrations.test.ts](panel/backend/src/db/__tests__/fresh-install-migrations.test.ts)（4 用例：空库全链路按序执行 29 迁移 + seed 调整后定价 + 无表跳过 + 存量库 up/down 行为），defect:sync 通过（8 条，P1×3 全部已转化）。**验证**：三端 tsc 0 错误；后端 vitest 709/709（+4 新用例）；daemon 20/20；前端 328/328；前端 build + 两端 bundle 成功；三端 dist 无 localhost:3000/127.0.0.1:3000 违规；check:version 12 源全绿 4.39.2；BUILD_ID 20260731-002。**部署**：s0701 七闸门过检（端口 3002/8080 一致 / systemd 文件存在 / /opt 目录分离 / dist 路径正确 / 停止旧部署）→ `deploy.sh update` exit 0 → 生产验证全绿：ExecStart 已切 node dist/index.js、双服务 active+enabled、Panel 3002 ok、HTTPS 3001 ok、HTTP 3000→301、公网 `/api/version`=4.39.2、Daemon connected 4.39.2、BUILD 20260731-002 已部署。**遗留（不阻断）**：375×812 真机复核（v4.38.0 声明项）未闭合；部署副作用（游戏实例因 daemon 重启标记 stopped 需手动启动，v4.39.1 已登记为架构改进项）。
- **v4.39.1 项目体检整改落地 + git 基线建立（已部署，2026-07-31）**：基于项目体检报告（s0602）执行全面整改。**①DEF-007 修复**：`scheduler-init.ts:546` + `operationsService.ts:386` 两处 `db('backups')` 误用修正为 `db('backup_records')`（表名漂移导致 ALERT_BACKUP_HEALTH_CHECK 每日报错且备份健康检查永远失效），后端 tsc 0 错误 + vitest 705/705 PASS。**②defect-log 全量转化**：DEF-001~006 经代码核实均已修复（npx tsx / build:release 拆分 / /docs 7 路由已注册 / barrel 回环已解 / deploy.sh exclude .env / migration hasColumn），逐条补转化产物与预防措施，DEF-007 录入并转化，`defect:sync` 校验通过 unconverted=0。**③版本源补齐**：deploy.sh DEPLOY_VERSION 4.39.0→4.39.1（并行会话遗漏），check:version 12 源全绿。**④git 基线建立**：v4.36.1 之后全部工作（120 文件，含并行会话 v4.39.1 Apple HIG 对齐产出）分 3 批提交——1f2a9df（后端 42 文件）/ adf20f6（前端 56 文件）/ 0a83e82（契约+文档+留痕 38 文件），工作区 clean。**⑤部署**：s0701 预检 6 闸门通过（端口/.env/systemd/目录/dist/停止旧部署）→ `deploy.sh update` exit 0 → 生产验证全绿：Panel 3002 ok+4.39.1、HTTPS 3001 ok、HTTP 3000→301、Daemon 8080 ok+4.39.1、BUILD 20260731-001、Daemon WS 已连接。**部署副作用（已知行为）**：2 个游戏实例（工厂/我的世界测试）因 daemon 重启被标记 stopped，需用户在面板手动启动——根因是 daemon 无进程重认领机制（spawn 无 detached + systemd 默认 KillMode=control-group），每次部署都会重启所有实例，已登记为后续架构改进项。**遗留（不阻断）**：build pipeline 重构（`node dist/` 方案：tsconfig rootDir + @public 路径别名运行时解析 + .ts 扩展名）仍为 TODO，作为独立版本排期；375×812 真机复核（v4.38.0 声明项）未闭合。
