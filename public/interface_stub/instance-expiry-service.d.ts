 ...
 * 数据契约：
 *   - public/schema/server-schema.json（servers.expires_at / expiry_status / expiry_grace_until / billing_type）
 *   - public/schema/instance-renewals-schema.json（续费记录，v3-billing 扩展 billing_cycle_months / instance_type_snapshot）
 ...
 * v3-billing 扩展：servers.billing_type='vps_prepaid' 的实例由 instanceBillingService 接管计费，
 *                 本服务仅负责有效期状态机扫描与磁盘清理，renewInstance 委托计费给 instanceBillingService。
 *
 ...
 *   - INSTANCE_AUTO_RENEWAL (cron '0 3 * * *')：v3-billing 新增，VPS 自动续扣扫描（与 INSTANCE_GRACE_CLEANUP 同点触发，先续扣后清理）
 */
...
export interface InstanceExpiryService {
   ...
   *
   * v3-billing 路由分流：
   *   - servers.billing_type='vps_prepaid' → 委托 instanceBillingService.chargeInstanceRenewal
   *     （按 instance_type 月费 × 周期 × 折扣计费，duration_days 由 billing_cycle_months × 30 推导）
   *   - servers.billing_type=null → 走下方 V6 全可配定价流程（pack.business.instance.pricing）
   *
   * 流程（决策 V6 全可配定价，仅 billing_type=null 时走此路径）：
